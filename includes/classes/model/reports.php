...
class reports
{
  ...
  public static function add_filters_query($reports_id,$listing_sql_query, $prefix = '')
  {
    $reports_info_query = db_query("select * from app_reports where id='" . db_input($reports_id). "'");
    if($reports_info = db_fetch_array($reports_info_query))
    {
      ...
      {                
        ...
        {       
          $sql_query = fields_types::reports_query(array('class'=>$filters['type'],'filters'=>$filters,'sql_query'=>$sql_query,'prefix'=>$prefix));
        }
      }
  ...
  {  
    ...
    while($v = db_fetch_array($filters_query))
    {
      ...
      {
        $fitlers_values = reports::render_filters_values($v['fields_id'],$v['filters_values']); 
      }
