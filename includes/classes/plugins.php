...
class plugins
{
  ...
  static public function include_dashboard_with_selected_menu_items($reports_id)
  {
    ...
    if(count($app_plugin_menu)>0)
    {
      ...
      {
        //update records        
        $html .= '<li>' . link_to_modalbox(TEXT_EXT_UPDATE_RECORDS,url_for('ext/withs_elected/update','reports_id=' . $reports_id)) . '</li>';
      }
              
      if(users::has_access('create',$access_schema))
      {   
        //copy records           
        $html .= '<li>' . link_to_modalbox(TEXT_COPY_RECORDS,url_for('ext/withs_elected/copy','reports_id=' . $reports_id)) . '</li>';
        ...
        {          
          $html .= '<li>' . link_to_modalbox(TEXT_MOVE_RECORDS,url_for('ext/withs_elected/move','reports_id=' . $reports_id)) . '</li>';
        }
      }
    }
