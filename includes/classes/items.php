...
class items
{
  public static function get_choices_by_entity($entity_id,$parent_entity_id,$report_id=0)
  {
    ...
    while($item = db_fetch_array($items_query))
    {
      ...
      $choices[$path_info['full_path'] . '/' . $entity_id] = $parent_name . ($heading_field_id>0 ? self::get_heading_field_value($heading_field_id,$item) : $item['id']); 
    }
  ...
  public static function get_heading_field_value($heading_field_id,$item_info)
  {
    ...
    if(strlen($heading_field_value)==0)
    {
      return '';
    }
    ...
    {             
      switch($field_info['type'])
      {
        ...
        case 'fieldtype_dropdown_multiple':                
            return fields_choices::render_value($heading_field_value,$app_choices_cache,true);
          break;
        case 'fieldtype_grouped_users':
            $heading_field_value = substr($heading_field_value,0,strpos($heading_field_value,'|'));
            return fields_choices::render_value($heading_field_value,$app_choices_cache,true);
          break;  
            ...
            foreach(explode(',',$heading_field_value) as $item_id)
            {
              ...
              {   
                ...
                {             
                  ...
                  {
                    $output[] = self::get_heading_field_value($fields['id'],$item);               
                  }
                  else
                  {
                    $output[] = $item['id'];
  ...
  public static function render_info_box($entity_id,$item_id,$users_id=false)
  {
     ...
     * and skip reladted records when we build info box for singe user in email content     
     */         
  ...
  public static function render_content_box($entity_id,$item_id,$users_id = false)
  {
    ...
    $html = '';  
    $fields_query = db_query("select f.* from app_fields f where f.type in ('fieldtype_textarea','fieldtype_textarea_wysiwyg','fieldtype_attachments','fieldtype_image') and  f.entities_id='" . db_input($entity_id) . "' order by f.sort_order, f.name");
    while($field = db_fetch_array($fields_query))
    {   
      //check field access
      if(isset($fields_access_schema[$field['id']]))
