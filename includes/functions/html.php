<?php
  ...
  {         
    ...
    {            
      ...
      $html .= '<div>' . input_radiobox_tag($name,$k,$attributes) . ' <label for="' . $attributes['id'] . '">' . $v . '</label></div>'; 
    }
  ...
  function button_tag($value,$url,$is_dialog = true,$attributes=array(),$left_icon='', $right_icon='')
  {
    ...
    return '<button ' . ($is_dialog ? 'onClick="open_dialog(\'' . $url . '\'); return false;"':'onClick="location.href=\'' . $url . '\'"') . ' ' . tag_attributes_to_html($default,$attributes) . '>' . $left_icon . $value . $right_icon . '</button>';
  }
  
  function button_icon($title,$class,$url,$is_dialog = true)
  {            
    return '<a ' . tag_attributes_to_html(array('title'=>$title)) . ' class="btn btn-default btn-xs purple" href="#" ' . ($is_dialog ? 'onClick="open_dialog(\'' . $url . '\'); return false;"':'onClick="location.href=\'' . $url . '\'"') . '><i class="' . $class . '"></i></a>';        
  }
