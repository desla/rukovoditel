...
//check if user logged
  if (!app_session_is_registered('app_logged_users_id') and !in_array($_GET['module'],array('users/login','users/restore_password','users/ldap_login','ext/calendar/icalexport'))) 
  { 
  ...
  {            
    ...
    if($user = db_fetch_array($user_query))
    {
      ...
      $app_user = array('id'=>$user['id'],
                        ...
                        'name'=> (CFG_APP_DISPLAY_USER_NAME_ORDER=='firstname_lastname' ? $user['field_7'] . ' ' . $user['field_8'] : $user['field_8'] . ' ' . $user['field_7']),
