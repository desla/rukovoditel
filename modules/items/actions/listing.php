...
$count_sql = "select count(*) as total from app_entity_" . $current_entity_id . " e "  . $listing_sql_query_join . " where e.id>0 " . $listing_sql_query;
$listing_split = new split_page($listing_sql,$_POST['listing_container'],$count_sql);
...
while($item = db_fetch_array($items_query))
{
  ...
  foreach($listing_fields as $field)
  {
    ...
    {     
      ...
      $html .= '
          <td class="' . $field['type'] . ' item_heading_td"><a href="' . url_for('items/info', 'path=' . $path . '&redirect_to=subentity') . '">' . fields_types::output($output_options) . '</a>
      ';
