      ...
      <div>
        <table width="100%">
          <tr>
            ...
            <td align="right">' . (!in_array($v['type'],fields_types::get_users_types()) ? button_icon_edit(url_for('entities/fields_form','id=' . $v['id']. '&entities_id=' . $_GET['entities_id']. '&redirect_to=forms')) . ' ' . button_icon_delete(url_for('entities/fields_delete','id=' . $v['id'] . '&entities_id=' . $_GET['entities_id']. '&redirect_to=forms')):'') . '</td>
          </tr>
        </table>
      </div>
    </li>';
